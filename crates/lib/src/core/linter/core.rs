...
impl Linter {
    ...
    pub fn set_rule_set(&self, rule_set: &RuleSet) -> Result<(), SQLFluffUserError> {
        self.rulepack
            .set(rule_set.get_rulepack(&self.config)?)
            .map_err(|_| SQLFluffUserError::new("Can not call `Linter::set_rule_set` more then once or after `get_rulepack` has been called".to_string()))
    }
